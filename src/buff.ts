...
import { $coinmaster, $effect, $familiar, $item, $items, $location, $monster, $skill, $slot, $stat, SourceTerminal, get, have, set, Macro,} from "libram";

import { tryUse, eatPizza, ensureEffect, ensureItem, ensureNpcEffect, ensurePotionEffect, getPropertyBoolean, hotTub, md_uniform, pullIfPossible, reminisce, setChoice, topupMP, tuneMoon, withMacro, ensureMp} from "./phredhccs-lib";
...
function synthBuffs(){
  tuneMoon(); // need to have gnomad access
  synth("stats");
  synth("statgain");
}
...
function hobelf() {
  ...
  if(!have($item`Wint-O-Fresh mint`)){
    ...
    reminisce($monster`elf hobo`);
    //reminisce($monster`hobelf`);
